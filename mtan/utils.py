...
def evaluate_classifier(model, test_loader, dec=None, args=None, classifier=None,
    ...
    for test_batch, label in test_loader:
        test_batch, label = test_batch, label
...
def get_mimiciii_data(args):
    ...
    data_objects = {"train_dataloader": train_dataloader,
                    ...
                    "input_dim": input_dim}
    return data_objects
from parsePKPD import read_in_data, random_timestep_sampling, generate_random_timesteps
def get_pkpd_data(args, device, q, flag = 1):
    combos, combo_to_key = read_in_data("data/pkpd/intensiveQD.csv")
    combo = combos[combo_to_key[1]]
    doses2train = [0.03]
    combo = random_timestep_sampling(combo, doses2train)
    data = []
    count = 0
    for dose in combo: #(record_id, tt, vals, mask, labels)
        for subject in combo[dose]:
            data.append((count, subject[:, 2], subject[:, 3], [1 if v else 0 for v in subject[:, 3]], None))
            count += 1
    return data
...
# =============================================================================
#     print()
#     print(train_dataset_obj.__getitem__(2)[0])
#     print(train_dataset_obj.__getitem__(2)[1].shape)
#     print(train_dataset_obj.__getitem__(2)[2].shape)
#     print(train_dataset_obj.__getitem__(2)[3].shape)
#     print(train_dataset_obj.__getitem__(2)[4])
#     print()
# =============================================================================
    ...
    record_id, tt, vals, mask, labels = train_data[0] #this is how data is stored in a tuple
