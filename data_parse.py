...
def read_in_data(filename):
    combos, combo_to_key = parse_csv(filename)
...
def random_timestep_sampling(combo, doses_to_train):
    ...
    for dose in doses_to_train:
        dose = "DOSE: " + str(dose)
        new_combo[dose] = np.zeros(combo[dose].shape)
        for i in range(100): #num_subjects
            tt_pk = generate_random_timesteps()
            tt_pd = generate_random_timesteps()
            for j in range(new_combo[dose].shape[1]):
                new_combo[dose][i][j] = combo[dose][i][j]
                new_combo[dose][i, :, 3] *= [1 if i in tt_pk else 0 for i in range(new_combo[dose].shape[1])]
                new_combo[dose][i, :, 4] *= [1 if i in tt_pd else 0 for i in range(new_combo[dose].shape[1])]
    return new_combo
...
def generate_random_timesteps():
    ...
    for day in range(7):
        psbl_timesteps[day] = [psbl_timesteps[day][i] for i in random.sample(range(48), 6)] #every 24 hours
