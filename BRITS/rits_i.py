...
class Model(nn.Module):
    ...
    def build(self):
        ...
        self.regression = nn.Linear(self.rnn_hid_size, 3)
        self.temp_decay = TemporalDecay(input_size = 3, rnn_hid_size = self.rnn_hid_size)
    ...
    def forward(self, data, direct):
        ...
        for t in range(SEQ_LEN):
            ...
            m = masks[:, t, :]
            d = deltas[:, t, :]
            #print(x.shape, m.shape, d.shape)
            gamma = self.temp_decay(d)
            #print(m.shape, x.shape, h.shape, gamma.shape)
            h = h * gamma
            x_h = self.regression(h)
            #print(m.shape, x.shape, x_h.shape)
            x_c =  m * x +  (1 - m) * x_h
