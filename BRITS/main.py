...
def train(model):
    ...
    data_iter = pkloader.get_loader(batch_size=args.batch_size)
    min_loss = float("inf")
    for epoch in range(args.epochs):
        model.train()
        #model.load_state_dict(torch.load("saved_model/rits.pt"))
        ...
        for idx, data in enumerate(data_iter):
            ...
            curr_loss = ret['loss'].item()
                
            run_loss += ret['loss'].item()
# =============================================================================
#             if curr_loss < min_loss:
#                 PATH = "saved_model/rits.pt"
...
def evaluate(model, val_iter):
    ...
    np.save('./result/{}_impute'.format(args.model), np.array(imputation))
    np.save('./result/{}_eval'.format(args.model),  np.array(eval_))
