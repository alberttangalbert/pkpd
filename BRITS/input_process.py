...
match = [re.findall('\d{6}',line) 
            for line in open('./raw/Outcomes-a.txt')] 
#id_ = match.group()
patient_ids = match
...
def parse_id(id_):
    ...
    #mask is true if there if value is not nan, basically true where all the training dataset (after 10% reduction)
    #have a value that exists
    masks = ~np.isnan(values)
    #eval mask is true if there if value is not nan, basically true where all the training dataset (10% reduction data)
    #have a value that exists. These are true if the data is nan and it is the value that you are going to predict
    ...
    #Evals: original dataset, values: dataset after 10% reduction, mask: mask of values, eval_mask: mask of data that 
    #was eliminited between evals and values, data to predict 
...
#%%
id_ = '132653'
print(out.loc[int(id_)])
data = pd.read_csv('./raw/set-a/{}.txt'.format('132653'))
data['Time'] = data['Time'].apply(lambda x: to_time_bin(x))
evals = []
# merge all the metrics within one hour
for h in range(48):
    x = parse_data(data[data['Time'] == h])
    evals.append(x)
#%%
evals = (np.array(evals) - mean) / std
#%%
...
delta = parse_delta(masks, "f")
#%%
rec = parse_rec(values, masks, evals, eval_masks, "foward")
