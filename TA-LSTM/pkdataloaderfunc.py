...
def random_timestep_sampling(combo, doses_to_train):
    ...
    for dose in doses_to_train:
        dose = "DOSE: " + str(dose)
        new_combo[dose] = np.zeros((100, 42, 6))
        for i in range(100):
            psbl_timesteps = generate_random_timesteps()
            new_combo[dose][i] = combo[dose][i,psbl_timesteps, :]
    return new_combo

def random_timestep_removal(pkdata):
    x, y, z = pkdata.shape

    #entire mask for all subjects
    emask = np.zeros(pkdata.shape)
    newpkdata = np.zeros(pkdata.shape)
    for i in range(100):
        mask = np.ones(pkdata[0].shape)
        #print(mask.shape)
        torem = generate_random_timesteps()
        psbl_timesteps = [0 if i not in torem else 1 for i in range(337)]
        mask[:,z - 1] = mask[:,z - 1] * psbl_timesteps
        
        for j in range(337):
            for k in range(z):
                if mask[j, k] == 1: newpkdata[i, j, k] = pkdata[i, j, k]
                else: newpkdata[i, j, k] = np.nan
                
        emask[i] = mask
    return newpkdata, emask
...
def create_data_XY(combo, doses_to_train, features_to_train, feature_to_predict):
    doseKey = "DOSE: " + str(doses_to_train[0])
    dataX = np.zeros((len(doses_to_train) * num_subjects, len(combo[doseKey][0,:,0]), features_to_train))
    dataY = np.zeros((len(doses_to_train) * num_subjects, len(combo[doseKey][0,:,0]), 1))
    for i in range(len(doses_to_train)):
        doseKey = "DOSE: " + str(doses_to_train[i])
        dataX[i * 100:(i + 1) * 100, :, [0, 1, 2]] = combo[doseKey][:,:,[0, 1, 3]]
