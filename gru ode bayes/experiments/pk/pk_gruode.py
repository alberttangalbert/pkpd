...
parser = argparse.ArgumentParser(description="Running GRUODE on PK")
parser.add_argument('--model_name', type=str, help="Model to use", default="pk_gruode")
parser.add_argument('--dataset', type=str, help="Dataset CSV file", default="gru_ode_bayes/datasets/pk/processed_isample_pk.csv")
parser.add_argument('--jitter', type=float, help="Time jitter to add (to split joint observations)", default=0)
parser.add_argument('--seed', type=int, help="Seed for data split generation", default=815)
...
parser.add_argument('--no_impute',action="store_true",default = True) #might need to change
...
train_idx, val_idx = train_test_split(np.arange(metadata["N"]),test_size=0.1, random_state=args.seed)
...
dl     = DataLoader(dataset=data_train, collate_fn=data_utils.custom_collate_fn, shuffle=True, batch_size=1,num_workers=0)
dl_val = DataLoader(dataset=data_val, collate_fn=data_utils.custom_collate_fn, shuffle=False, batch_size=len(data_val),num_workers=0)
...
for epoch in range(epoch_max):
    ...
    for i, b in tqdm.tqdm(enumerate(dl)):
        ...
        p_vec = p_vec.cpu().detach()
        for dim in range(2):
            m, v = torch.chunk(p_vec[:,0,:],2,dim=1)
            plt.plot(t_vec, m[:,dim].cpu().numpy())
...
print(model_name)
#%%
gru_ode_bayes.paper_plotting.plot_trained_model(model_name = "pk_gruode", data_type = "pk")
