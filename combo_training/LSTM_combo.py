...
for combo in dataXY.keys():
    ...
    for i in range(1, len(dosages) - 1):
        ...
        bestmodel_filename = "LSTM_epochs{0}_train{1}.h5".format(num_epochs, dose_train)
    ...
    bestmodel_filename = "LSTM_combo{2}_epochs{0}_train{1}.h5".format(num_epochs, dose_train, count)
    bestmodel_filepath = os.path.join(path, bestmodel_filename)
...
model_stateless.add(LSTM(units=cellunits, return_sequences=True, 
...
bestmodel_filepath = r"C:\Users\alber\Code\Python\PKPD\pkpd_combo\combo1\LSTMwo_epochs1000_train003_01_03_10_30.h5"
model_stateless.load_weights(bestmodel_filepath)
#%%
predict = model_stateless.predict(dataXY["Combo1: [0.1, 0.1, 0.1, 1.0]"]["DOSE: 10.0"][0])
fig11 = plt.figure()
title = "Combo1 - 0.03, 0.1, 0.3 to predict 1 GRU, 1 Dense"
plt.title(title)
predictX = denorm(dataXY["Combo1: [0.1, 0.1, 0.1, 1.0]"]["DOSE: 1.0"][0], 
                  maximum["Combo1: [0.1, 0.1, 0.1, 1.0]"]["DOSE: 1.0"][0][2],
                  minimum["Combo1: [0.1, 0.1, 0.1, 1.0]"]["DOSE: 1.0"][0][2])
predictY = denorm(predict[0,:,], maximum["Combo1: [0.1, 0.1, 0.1, 1.0]"]["DOSE: 10.0"][0][3], 
                 minimum["Combo1: [0.1, 0.1, 0.1, 1.0]"]["DOSE: 1.0"][0][3])
plt.plot(predictX[0], predictY)
true = denorm(dataXY["Combo1: [0.1, 0.1, 0.1, 1.0]"]["DOSE: 1.0"][1], 
              maximum["Combo1: [0.1, 0.1, 0.1, 1.0]"]["DOSE: 1.0"][0][3],
              minimum["Combo1: [0.1, 0.1, 0.1, 1.0]"]["DOSE: 1.0"][0][3])
plt.plot(true[0])
