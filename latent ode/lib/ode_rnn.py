...
class ODE_RNN(Baseline):
    def __init__(self, input_dim, latent_dim, device = torch.device("cuda:0"),
...
# =============================================================================
#         if (random.choice([i for i in range(20)]) == 0):
#             pred = outputs.detach().cpu().numpy()
#             act =  data.detach().cpu().numpy()
#             #print(pred.shape)
#             fig = plt.figure()
#             plt.plot(pred[0, 0, :,0])
#             plt.plot(act[0, :,0])
#             count = 0
#             while exists('results/{0}.png'.format(count)):
#                 count+=1
