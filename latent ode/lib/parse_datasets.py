...
from data_parse import read_in_data, normalize_doses, create_data_XY, random_timestep_sampling, batch_rts
...
def parse_datasets(args, device):
    ...
    def pk_collate_fn(batch, args = args, device = device, data_type = "train"):
        batch = torch.stack(batch)
        batch = batch_rts(batch)
        batch = batch.unsqueeze(0)
        time_steps = batch[:,:,2]
        time_steps = time_steps.squeeze()
        #time_steps = time_steps.unsqueeze(-1)
        #torch.reshape(time_steps, (len(time_steps), 42, 1))
        #torch.reshape(time_steps, (42, 1))
        batch = batch[:,:,3].unsqueeze(-1)
        #batch = batch.squeeze()
    ...
    if dataset_name == "pkpd":
        dataset_obj = PK_Dynamics(root='data', doses = [0.03], generate=False, device = device)
        dataset = dataset_obj.get_dataset()#[:args.n]
        #time_steps = dataset[:,:,1]
        #print(dataset.shape)
        ...
        train_y, test_y = utils.split_train_test(dataset, train_fraq = .99)
        n_samples = len(dataset)
        input_dim = dataset.size(-1)
        batch_size = 1
        #min(args.batch_size, args.n)
        train_dataloader = DataLoader(train_y, batch_size = batch_size, shuffle=False,
            collate_fn= lambda batch: pk_collate_fn(batch, data_type = "train"))
        test_dataloader = DataLoader(test_y, batch_size = n_samples, shuffle=False,
            collate_fn= lambda batch: pk_collate_fn(batch, data_type = "test"))
