...
class PersonActivity(object):
	...
	def __init__(self, root, download=False,
		...
		if device == torch.device("cuda:0"):
			self.data = torch.load(os.path.join(self.processed_folder, self.data_file), map_location='cuda:0')
		else:
...
def variable_time_collate_fn_activity(batch, args, device = torch.device("cuda:0"), data_type = "train"):
	"""
