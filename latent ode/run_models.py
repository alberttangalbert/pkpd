...
# =============================================================================
    ...
    if args.classif and (args.dataset == "hopper" or args.dataset == "periodic" or args.dataset == "pkpd"):
    ...
    QDcombos, combo_to_key = read_in_data("data/pk_dynamics/intensiveQD.csv")
    #QD combos only 1 combo 6
    combo_to_train_data = QDcombos[combo_to_key[1]]
    combo_to_train_data = normalize_doses(combo_to_train_data, [0.03])
    dataX, _ = create_data_XY(combo_to_train_data, [0.03], 3, 3)
    for itr in range(1, num_batches * (args.niters + 1)):
        ...
        if itr % (n_iters_to_viz * num_batches) == 0:
            ...
            from data_parse import batch_rts
            
            time_steps = torch.from_numpy(dataX[i,:,2]).float().to(device)
            truth = torch.from_numpy(dataX[i,:,3].T.reshape(1, 337,1)).float().to(device)
            mask = torch.from_numpy((np.ones((1, 337, 1)))).double().float().to(device)
            outputs, extra_info = model.get_reconstruction(time_steps, 
                                                            truth, 
                                                            time_steps, 
                                                            mask = mask)
            fig = plt.figure()
            plt.plot(outputs[0,0,:,0].cpu().detach().numpy())
            plt.plot(dataX[i,:,3])
        
        
            while exists('whole/{0}.png'.format(count)):
                count+=1
            fig.savefig('whole/{0}.png'.format(count))      
    torch.save({
        'args': args,
        'state_dict': model.state_dict(),
    }, ckpt_path)
...
dataX, _ = create_data_XY(combo_to_train_data, [0.03], 3, 3)
#%%
ckpt_path = 'experiments/experiment_78021.ckpt'
checkpt = torch.load(ckpt_path)
#print(checkpt["args"])
model.load_state_dict(checkpt["state_dict"])
#%%

time_steps = dataX[10,:,2]
truth = dataX[10,:,3].T.reshape(1, 337,1)
print(truth)
mask = torch.from_numpy((np.ones((1, 337, 1))))
outputs, extra_info = model.get_reconstruction(torch.tensor(time_steps).float().cuda(), 
                                               torch.tensor(truth).float().cuda(), 
                                               torch.tensor(time_steps).float().cuda(), 
...
for item in batch_dict:
    if item != "mode" and item!="labels" and item!="mask_predicted_data":
        data[item] = (batch_dict[item].cpu().numpy())
    #data_to_predict_numpy = batch_dict["observed_data"].cpu().numpy()
#%%
